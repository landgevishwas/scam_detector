...
class ScamDetector:
    ...
    def detect_batch(self, messages: List[str]) -> List[Dict[str, Any]]:
       ...
       #messages = ["Message 1", "Message 2", "Message 3"]  # Example messages for testing
        ...
        return results
        #results is a list of dictionaries, where each dictionary contains the detection result for a corresponding message. Each dictionary includes keys such as "label", "reasoning", "intent", and "risk_factors". The method processes each message in the input list, applies the detect method, and handles any exceptions by appending an error result to the results list. Finally, it logs the number of successful detections and returns the results.